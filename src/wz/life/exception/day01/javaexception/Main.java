package wz.life.exception.day01.javaexception;
...
//        上述代码仍然会得到编译错误，但这一次，
//        编译器提示的不是调用return s.getBytes("GBK");的问题，
//        而是byte[] bs = toGBK("中文");。
...
//        修复方法是在main()方法中捕获异常并处理：
        ...
        //可见，只要是方法声明的Checked Exception，
        // 不在调用层捕获，也必须在更高的调用层捕获。所有未捕获的异常，
        // 最终也必须在main()方法中捕获，不会出现漏写try的情况。
...
//    并且准确地指出需要捕获的语句是return s.getBytes("GBK");。意思是说，像UnsupportedEncodingException这样的Checked Exception，必须被捕获。
...
//    }
    ...
    // 就必须捕获UnsupportedEncodingException。
